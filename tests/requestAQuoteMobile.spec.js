...
test.describe('Request a Quote Form - Required Fields Check', () => {
  ...
  test('Verify that the form includes a honeypot or a captcha to make the form less susceptible to bots', async ({ page }) => {
    ...
    console.log('Clicking on "Sign Holders"...');
    await page.check('input[name="input_7_3"]');

    console.log('Selecting "Google" from the dropdown...');
    await page.selectOption('select[name="input_9"]', 'Google');
  ...
  test('Verify that a confirmation message is displayed when the form is submitted', async ({ page }) => {
    ...
    await page.waitForSelector('#gform_confirmation_message_7', { timeout: 10000 });

    console.log('Checking for confirmation message...');
    const confirmationMessage = await page.$('#gform_confirmation_message_7');
  ...
  test('Verify form submission is recorded in the CMS', async ({ page }) => {
    ...
    console.log('Clicking the admin menu button...');
    await page.click('li#wp-admin-bar-menu-toggle a.ab-item');
    ...
    await page.waitForSelector('a.wp-first-item[href="admin.php?page=gf_edit_forms"]', { timeout: 10000 });
    await page.click('a.wp-first-item[href="admin.php?page=gf_edit_forms"]');

    console.log('Selecting the "Request A Quote" form...');
    await page.waitForSelector('a[href="?page=gf_edit_forms&id=7"]', { timeout: 10000 });
    await page.click('a[href="?page=gf_edit_forms&id=7"]');
