...
test.describe('Where to Buy Form - Automated Checks', () => {
  ...
  test('Verify store addresses are displayed and clickable for a valid ZIP code', async ({ page }) => {
    ...
    console.log('Waiting for store addresses or "No results found" message...');
    const storeAddresses = await page.waitForSelector('div.stores div[id^="store-"], p:has-text("No results found")', { timeout: 15000 });
    ...
    const storeAddressElements = await page.$$('div.stores div[id^="store-"] button');

    if (storeAddressElements.length > 0) {
      console.log(`Number of store addresses found: ${storeAddressElements.length}`);
      for (const storeAddress of storeAddressElements) {
  ...
  test('Verify results for each radius selection with a valid ZIP code', async ({ page }) => {
    ...
    for (const radius of radiusOptions) {
      ...
      if (storeAddressElements.length > 0) {
        console.log(`Number of store addresses found for radius ${radius}: ${storeAddressElements.length}`);
        for (const storeAddress of storeAddressElements) {
          const isVisible = await storeAddress.isVisible();
        ...
        console.log(`No results found for radius ${radius}.`);
      }
  ...
  test('Verify clicking on an address updates the map', async ({ page }) => {
    ...
    if (storeAddressElements.length > 0) {
      ...
      const firstStoreAddress = storeAddressElements[0];
